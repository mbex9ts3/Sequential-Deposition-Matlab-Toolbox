...
logiSt1 = false(6,1);
for i = 1:6
    logiSt1(i) = strcmp(testString{i}, distMod1);
end

if ~isequal(sum(logiSt1), 1)
...
logiSt2 = false(6,1);
for i = 1:6
    logiSt2(i) = strcmp(testString{i}, distMod2);
end

if ~isequal(sum(logiSt2), 1)
    error('Input distribution not recognized');
end
...
% initialize
distroIn = logical(round(rand));

% expo flag (one input)
if distroIn == false
    distroFlag = distroFlag1;
    distMod = distMod1;
    param1 = param1_1;
    param2 = param1_2;
    minDg = minDg1;
