function [centroids, radii] = drop_roll_periodic(x_bounds, y_bounds, layers, distMod, param1, param2, param1_sc, param2_sc, minDg, maxDg, vizFlag)
...
param2In = param2(1);
layersIn = layers(1);
param1_scIn = param1_sc(1);
param2_scIn = param2_sc(1);
maxDgIn = maxDg(1);
minDgIn = minDg(1);
currentLayer = 1;

% test distribution input
testString = [{'Exponential'}; {'Normal'}; {'Uniform'};  {'Lognormal'}; {'Weibull'}];
logiSt = false(5,1);
for i = 1:5
    logiSt(i) = strcmp(testString{i}, distroIn);
end
...
while max(circC(:,2)) < y_bounds(2)
    ...
    if max(circC(:,2)) > layersIn
        currentLayer = currentLayer+1;
        distroIn = distMod{currentLayer};
        param1In = param1(currentLayer);
        param2In = param2(currentLayer);
        layersIn = layers(currentLayer);
        param1_scIn = param1_sc(currentLayer);
        param2_scIn = param2_sc(currentLayer);
        maxDgIn = maxDg(currentLayer);
        minDgIn = minDg(currentLayer);
        ...
        disp(['building layer...' num2str(currentLayer)]);
    end
end
